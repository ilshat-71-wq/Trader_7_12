"""Trader_7_12 Pro — SPOT-only read-only radar UI."""
...
def _activity_label(item):
    ratio = float(item.get("spot_session_activity_ratio", item.get("spot_money_ratio", 0)) or 0)
    if ratio >= 3:
        return f"ОЧЕНЬ ВЫСОКАЯ ({ratio:.2f}×)"
    if ratio >= 1.5:
        return f"ВЫСОКАЯ ({ratio:.2f}×)"
    if ratio >= 0.75:
        return f"НОРМАЛЬНАЯ ({ratio:.2f}×)"
    if ratio > 0:
        return f"НИЖЕ НОРМЫ ({ratio:.2f}×)"
    return "НЕТ ДАННЫХ"
...
class MeltingClocksWidget(QWidget):
    ...
    def _draw_clock(self, p, x, y, r, melt, angle):
        ...
        p.setPen(Qt.NoPen)
        p.drawEllipse(QPointF(0, 0), r * .06, r * .06)
        p.restore()
    ...
    def paintEvent(self, event):
        ...
        bg.setColorAt(.55, QColor("#171c21"))
        bg.setColorAt(1, QColor("#0f1418"))
        p.fillRect(rect, bg)
        sweep = (math.sin(self.phase * .55) + 1) / 2
        x = rect.left() + sweep * rect.width()
        band = QLinearGradient(x - 150, 0, x + 150, 0)
        band.setColorAt(0, QColor(190, 214, 120, 0))
        band.setColorAt(.5, QColor(190, 214, 120, 22))
        band.setColorAt(1, QColor(190, 214, 120, 0))
        p.fillRect(rect, band)
        w, h = rect.width(), rect.height()
        bob = math.sin(self.phase) * 7
        sway = math.sin(self.phase * .7) * 9
        base = min(w, h)
        ...
        p.setPen(QColor("#aab57d"))
        p.setFont(QFont("Helvetica Neue", 12, QFont.Weight.Medium))
        p.drawText(rect.adjusted(0, 0, 0, -22), Qt.AlignHCenter | Qt.AlignBottom, "АНАЛИЗ SPOT-РЫНКА")
        p.setPen(QColor("#727b68"))
        p.setFont(QFont("Helvetica Neue", 10))
        p.drawText(rect.adjusted(0, 0, 0, -7), Qt.AlignHCenter | Qt.AlignBottom, "SPOT  •  ЛИКВИДНОСТЬ  •  RS  •  SETUP")
        p.end()
...
class TraderWindow(QWidget):
    """Read-only SPOT radar with live Moscow time."""

    def __init__(self, scanner_enabled=True):
        super().__init__()
        self.setWindowTitle("Trader_7_12 Pro — SPOT-радар")
    ...
    def init_ui(self):
        ...
        self.subtitle = QLabel("SPOT • ЛИКВИДНОСТЬ • RS • SETUP")
        self.subtitle.setAlignment(Qt.AlignCenter)
        self.subtitle.setStyleSheet("font-size:13px;color:#89939d;padding:1px")
        self.session_label = QLabel()
        self.session_label.setAlignment(Qt.AlignCenter)
        self.session_label.setStyleSheet("font-size:20px;font-weight:700;color:#cbd1d7;padding:4px")
        self.clock_label = QLabel()
        self.clock_label.setAlignment(Qt.AlignCenter)
        self.clock_label.setStyleSheet("font-size:14px;color:#9fa8b1;padding-bottom:4px")
        self.scan_button = QPushButton("●  СКАНИРОВАТЬ SPOT-РЫНОК")
        ...
        self.result_stack.addWidget(self.result_box)
        self.result_stack.addWidget(self.scan_visual)
        self.result_stack.setCurrentWidget(self.result_box)
        if not self.scanner_enabled:
            ...
            self.result_box.setText("БКС ПОДКЛЮЧЁН\n\nНажмите «СКАНИРОВАТЬ SPOT-РЫНОК».\nРадар выберет TOP 2–3 базовых актива по текущей рыночной ситуации.")
        layout = QVBoxLayout()
        layout.setContentsMargins(18, 14, 18, 18)
        layout.setSpacing(6)
        for widget in (self.title, self.subtitle, self.session_label, self.clock_label, self.scan_button, self.result_stack):
    ...
    def _animate_scan(self):
        ...
        self.scan_button.setText("●  ИДЁТ АНАЛИЗ SPOT")
    ...
    def _stop_scan_animation(self, text="●  СКАНИРОВАТЬ SPOT-РЫНОК"):
    ...
    def _scan_finished(self, results):
        ...
        lines = [
            ...
            "═" * 78,
            "TRADER_7_12 PRO — TOP 2–3 БАЗОВЫХ АКТИВА",
            "═" * 78,
            "",
        ...
        if not results:
            lines.extend([
                ...
                "",
                "Нет базового актива, который прошёл обязательные SPOT-проверки в текущей сессии.",
            ])
        else:
        ...
        lines.extend([
            "",
            "СТРАТЕГИЯ СЕССИИ: ПРОДОЛЖЕНИЕ / СИЛА-СЛАБОСТЬ / АКТИВНОСТЬ",
            "",
            "ВАЖНО: это АНАЛИТИЧЕСКИЙ SPOT-РАДАР.",
            "Пользователь самостоятельно выбирает торговый инструмент, смотрит график и принимает решение о входе.",
