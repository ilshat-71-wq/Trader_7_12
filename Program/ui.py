...
class TraderWindow(QWidget):
    """Read-only radar with live Moscow clock and animated scan status."""
    ...
    def init_ui(self):
        ...
        self.scan_status.setStyleSheet("font-size: 18px; font-weight: 800; padding: 7px; border-radius: 8px;")
    ...
    def _update_session_header(self):
        ...
        self.clock_label.setText(f"{info.get('date', '—')}  •  МСК {info.get('time', '—')}  •  {market_state}")
        subtitles = {
            "PRE_OPEN": "ПРЕ-ОТКРЫТИЕ • 06:50–07:00 МСК • ПОДГОТОВКА",
            "MORNING": "УТРЕННИЙ РАДАР • 07:00–10:00 МСК • ОСНОВНОЕ ОКНО",
            "MAIN": "ОСНОВНАЯ СЕССИЯ • 10:00–19:00 МСК • КОНТРОЛЬ РЫНКА",
            "EVENING": "ВЕЧЕРНИЙ РАДАР • 19:00–23:50 МСК • КОНТРОЛЬ ВЕЧЕРНЕЙ СЕССИИ",
            "CLOSED": "РЫНОК ЗАКРЫТ • 23:50–06:50 МСК",
        }
        self.subtitle.setText(subtitles.get(session, "РЫНОЧНЫЙ РАДАР"))
    ...
    def _animate_scan(self):
        ...
        text_color, background = shades[self.animation_step]
        self.scan_status.setText(f"🔎  СКАНИРОВАНИЕ РЫНКА{dots}  •  ПРОЦЕСС ИДЁТ")
        self.scan_status.setStyleSheet(f"font-size: 18px; font-weight: 800; padding: 7px; border-radius: 8px; color: {text_color}; background: {background};")
