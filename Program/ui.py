...
class TraderWindow(QWidget):
    """Large scanner-only morning/session dashboard. No risk or execution controls."""
    ...
    def run_market_scan(self):
        ...
        self.result_box.setText("📡  СКАНИРОВАНИЕ РЫНКА...\n\nBCS → SPOT → ЛИКВИДНОСТЬ → СИЛА/СЛАБОСТЬ → СЕТАП → ФЬЮЧЕРС")
        self.scan_button.setEnabled(False)
    ...
    def _scan_finished(self, results):
        ...
        if not results:
            self.result_box.setText("🌙  ГОТОВЫХ КАНДИДАТОВ НЕТ\n\nНи один инструмент не прошёл обязательные проверки текущей сессии.")
            return
        ...
        for index, item in enumerate(results, 1):
            ...
            output.extend([
                ...
                f"СЕССИОННАЯ ОЦЕНКА: {_number(item.get('session_score'), 1)} / 100",
                f"ОЦЕНКА:            {_number(item.get('candidate_score'), 1)} / 100",
                f"RS:                {_rs_label(item)}",
                f"АКТИВНОСТЬ SPOT:   {_number(item.get('spot_money_ratio'), 2)}× среднего",
                ...
                f"ТРИГГЕР УРОВНЯ:     {_number(item.get('entry_trigger') or item.get('previous_high') or item.get('previous_low'), 4)}",
                "",
                "─" * 78,
            ])
        ...
        output.extend([
            "",
            f"СТРАТЕГИЯ СЕССИИ: {item.get('session_strategy', 'СЕССИОННЫЙ РАДАР')}",
            "",
