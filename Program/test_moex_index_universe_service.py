...
class MoexIndexUniverseServiceTests(unittest.TestCase):
    def test_extracts_tickers_from_standard_iss_table(self):
        ...
        self.assertEqual(MoexIndexUniverseService._extract_tickers(payload), {"SBER", "LKOH", "NVTK"})

    def test_filter_mappings_keeps_only_current_imoex_constituents(self):
        service = MoexIndexUniverseService(request_get=lambda *args, **kwargs: FakeResponse({}))
        ...
        self.assertEqual([item["spot_ticker"] for item in result], ["SBER", "LKOH"])
