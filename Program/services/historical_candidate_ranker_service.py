...
class HistoricalCandidateRankerService:
    ...
    def _spot_readiness_score(cls, row):
        """Score readiness from SPOT setup state only; futures confirmation is never a gate."""
    ...
    @classmethod
    def score(cls, row):
        """Calculate historical opportunity score from SPOT-only evidence."""
        direction = str(row.get("direction") or "").upper()
        ...
        setup_score = {
            ...
            "FIRST_PULLBACK": 11.0,
            "FIRST_REBOUND": 10.0,
        }.get(setup, 4.0 if setup != "NONE" else 0.0)
        spot_liquidity_score = cls._liquidity_score(
            row.get("average_daily_money"),
            weight=10.0,
        )

        # IMPORTANT: futures turnover, futures price and futures confirmation
        # are deliberately absent. They are outcome/mapping diagnostics only.
        score = (
            readiness_score
            + trend_score
            + move_score
