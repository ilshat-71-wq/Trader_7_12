...
class MorningReplayService:
    ...
    def _spot_setup_service(self, trading_date):
        ...
        if service is None:
            service = SpotFirstPullbackService(history_service=self.history_service, session_service=_ReplaySessionAdapter(trading_date))
    ...
    def load_candles(self, ticker, class_code, trading_date, end_time):
        ...
        start_moscow = datetime.combine(trading_date, self.MORNING_START, tzinfo=self.MOSCOW_TZ)
        end_moscow = datetime.combine(trading_date, end_time, tzinfo=self.MOSCOW_TZ)
        if end_moscow < start_moscow:
            return []
        candles = self.history_service.load(ticker, class_code, start_time=start_moscow.astimezone(timezone.utc), end_time=end_moscow.astimezone(timezone.utc), timeframe_minutes=5)
        result = []
        for candle in candles:
            dt = self.history_service.to_moscow(candle.get("time"))
            if dt is not None and self.MORNING_START <= dt.time() <= end_time:
    ...
    def calculate_momentum(self, candles):
        ...
        try:
            ...
            previous_high = previous_low = 0.0
        return self.momentum_service.analyze(current, average_volume=average_volume, average_money_volume=average_money_volume, previous_high=previous_high, previous_low=previous_low)
    ...
    def replay_setup(self, ticker, class_code, direction, trading_date, checkpoints=None):
        ...
        for checkpoint in checkpoints:
            ...
            candles = self.load_candles(ticker, class_code, trading_date, checkpoint)
            momentum = self.calculate_momentum(candles)
            setup = setup_service._detect_long(candles) if direction == "LONG" else setup_service._detect_short(candles)
            current_price = 0.0
            if candles:
                try:
    ...
    def print_results(results):
        ...
        print("=" * 140)
        print(f"{'TIME':<8}{'DIR':<7}{'MOM':>7}{'MOMENTUM':<15}{'VOL R':>8}{'MONEY R':>9}{'SETUP':<18}{'STATE':<10}{'TRIGGER':>12}{'QUALITY':>9}")
        print("-" * 140)
        for item in results:
            print(f"{item['checkpoint']:<8}{item['direction']:<7}{item['momentum_score']:>7}{item['momentum_signal']:<15}{item['volume_ratio']:>8.2f}{item['money_volume_ratio']:>9.2f}{item['setup']:<18}{item['setup_state']:<10}{item['entry_trigger']:>12.4f}{item['setup_quality_score']:>9.1f}")
        print("=" * 140)
