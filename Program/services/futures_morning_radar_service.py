...
class FuturesMorningRadarService:
    ...
    def scan(self, mappings=None, limit=None):
        ...
        results=[]; radar_cache={}; money_cache={}
    ...
    def _sort_key(item):
        ...
        return (1,activity,money,radar_score) if status=="OK" else (0,activity,money,radar_score)

    @staticmethod
    def print_results(results):
        print(); print("="*145); print("TRADER_7_12 PRO - SPOT MONEY -> FUTURES SESSION RADAR"); print("="*145); print()
        print(f"{'RANK':<6}{'FUTURES':<12}{'SPOT':<9}{'MONEY':>16}{'AVG MONEY':>16}{'RAW':>8}{'PACE':>9}{'EXPIRY':<12}{'DIR':<8}{'RADAR':>8}  SESSION")
        print("-"*145)
        for item in results:
            print(f"{item.get('rank','-'): <6}{item.get('futures_ticker','-'): <12}{item.get('spot_ticker','-'): <9}{float(item.get('spot_money_volume',0) or 0):>16,.0f}{float(item.get('spot_average_daily_money',0) or 0):>16,.0f}{float(item.get('spot_money_ratio',0) or 0):>8.2f}{float(item.get('spot_session_activity_ratio',0) or 0):>9.2f}{item.get('futures_expiry','-'): <12}{item.get('direction','-'): <8}{float(item.get('radar_score',0) or 0):>7.2f}  {item.get('market_session','-')}")
        print(); print("RAW = current session money / daily average money."); print("PACE = current session money / prorated daily baseline for elapsed session time."); print("No trade execution is performed by this service."); print("="*145)
