...
В UI используется **реальная Qt-таблица с отдельными колонками**, а не текстовая псевдотаблица с выравниванием пробелами. Колонки имеют стабильные размеры, числовые значения выравниваются вправо, строки не переносятся, таблица поддерживает горизонтальную прокрутку при недостатке ширины.
...
- `RS` — `PRICE Δ% − IDX Δ%`, в процентных пунктах.
- `₽/min` — средняя скорость реального денежного оборота с начала текущей торговой сессии.
- `Session` — накопленный реальный денежный оборот с начала текущей торговой сессии **до момента сканирования**.
- `15m` — реальный денежный оборот последних 15 минут.
- `Accel` — изменение скорости потока относительно предыдущего полного 15-минутного окна.

**UI-инвариант:** новый слой таблицы изменяет только представление. Расчёты, qualification, ranking, порядок `results` и значения данных не изменяются.
...
BR, RI, SI, золото, нефть, газ и валютные продукты не обязаны присутствовать в TOP20 при каждом сканировании: их место определяется реальным текущим оборотом и OI. Полный universe при этом не теряется — TOP20 является только представлением ликвиднейших front-контрактов.
...
22. SPOT и Futures OI выводятся как реальные Qt-таблицы с единым стилем; UI-рефакторинг не меняет расчёты и ranking.
